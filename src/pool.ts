...
// ⚠️ 这是**实测中位数**（2026-07-16，**上游**用 qwen3.7-max 连打 5 次：31.2/35.2/37.1/38.6/43.3s），
//    不是拍的。它只用于**给 Joe 看的估算**；真正的执行闸是 RoundBudget（看表，不用这个数）。
//    模型/提示词换了就会变 —— 变了这里的估算会不准，但**执行不会错**（那边看表）。
// 🔴 **这个数现在已经过期**（2026-08-31）：AirSonde 的写信模型改成 deepseek-v4-pro-0813，
//    与它被测量时的模型不是同一个。**没有改成猜的新数** —— 一个测过的旧数至少知道自己旧在哪，
//    一个拍的新数连这点都不知道。点火后第一批真发信要重测这 5 次并回填（已进点火 runbook 待办）。
