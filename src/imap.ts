...
import { countSocket } from "./subreq";
...
// ⚠️ 泛型参数写 ArrayBufferLike：流里读出来的 Uint8Array 可能背靠 SharedArrayBuffer，
//    而 `new Uint8Array(n)` 是 ArrayBuffer —— TS 5.7 之后这两个不再互相赋值。
//    我们只读字节，不在乎背后是哪种 buffer。**运行时零变化，纯类型标注。**
...
class Reader {
  ...
  // ⚠️ `Uint8Array<ArrayBufferLike>`：流读出来的可能背靠 SharedArrayBuffer，而 concat 返回的
  //    是 `Uint8Array<ArrayBuffer>` —— TS 5.7 之后这两个不再互相赋值。
  //    这里显式声明成 ArrayBufferLike：**只读字节，不在乎背后是哪种 buffer**。运行时零变化。
  constructor(private r: ReadableStreamDefaultReader<Uint8Array<ArrayBufferLike>>) {}
...
// ⭐ 间歇性 IMAP 故障判定（Lark 后端抖动/网络超时）：可**安全立即重试** + 告警走"连败阈值"而非每轮立吼。
//    2026-07-27 生产实测坐实：Lark Mail IMAP 后端会间歇吐 `<tag> NO internal server error`
//    （秒级内 FAIL→ok→ok→FAIL、挂点在 a2/a3 之间漂移 = 服务端抖动，不是我们凭据/配置）。
//    **只认这两类**：
//      · 会话超时（网络抖动）
//      · internal server error（Lark 后端临时错，SELECT/SEARCH/FETCH/LOGIN 任意命令上都可能出现）
//    真故障——登录被拒(认证失败)、FETCH 零正文、解析坏——**绝不纳入**：它们要立即吼、重试也没意义。
//    注意：这里不碰任何游标逻辑，只影响"重试一次"和"告警节奏"；零丢失底线（游标失败不推进）不受影响。
export function isTransientImapError(msg: string | undefined): boolean {
  const s = msg || "";
  return /会话超时/.test(s) || /internal server error/i.test(s);
}
...
export async function fetchNewMessages(env: Env, sinceUid: number, maxCount = IMAP_BATCH, timeoutMs = IMAP_SESSION_TIMEOUT_MS, account?: { user: string; pass: string }): Promise<FetchResult> {
  ...
  // ⚠️ 在调用点数，因为**包不住**：connect() 不经过 fetch，全局计量器看不见它。
  //    没有这一行时，构成表上「收回复」那一步是 ext+0 —— 那个 0 的意思是「看不见」，
  //    不是「没有」。（它算不算那 50 个额度是**另一个问题**，只能实测。）
  countSocket();
  ...
  const session = async (): Promise<FetchResult> => {
    ...
    if (newUids.length) {
      ...
      // ⭐⭐ 第四个洞（同一个函数里的第四个，总工找到的）：**`b0` 的 status 一直没人看。**
      //   我查了 a1 LOGIN、a2 SELECT、a3 SEARCH 的 status —— 唯独漏了这个。
      //
      //   后果不是"少一道校验"，是**我们把服务器的原话扔了，然后转述了一个自己的猜测**：
      //   Lark 很可能已经用 `b0 NO <真正的原因>` 明明白白说了，那行被 runCmd 收进 lines 然后丢掉，
      //   代码继续走到 literals=0，最后报一句**我们自己编的**"一封正文都没取到"。
      //   那句话看着很详细，**一个字的诊断价值都没有** —— 它只说结果是零，不说服务器说了什么。
      if (f.status !== "OK") {
        throw new Error(`IMAP FETCH 被拒（服务器原话）：${f.lines.slice(-1)[0]}`);
      }
      ...
      //
      //    ⭐ 2026-07-17 追加：**这条错误信息以前是我们自己编的结论，没有诊断价值。**
      //       总工手点了 4 次，两次是"FETCH 返回 OK 但零 literal" —— 而我们只报"一封没取到"，
      //       完全不知道服务器到底回了什么。**服务器在说话，我们把原话扔了然后转述自己的猜测。**
      //       所以现在把**响应原文**带出去：b0 状态行 + 前几行原始响应。
      //       这条链路（throw → r.error → reply_fail_last）才真的能拿来诊断。
      if (newUids.length && !messages.length) {
        const raw = f.lines.slice(0, 4).map((l) => l.slice(0, 120)).join(" ⏎ ");
        throw new Error(
          `IMAP FETCH 零正文：SEARCH 说有 ${newUids.length} 封（UID ${newUids.join(",")}），` +
          `FETCH 返回 ${f.status} 但一个 literal 都没有。**游标不推进**，下一班重试。\n` +
          `服务器原话（前 4 行）：${raw}\n` +
