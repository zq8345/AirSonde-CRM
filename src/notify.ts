...
export function inboundCard(d: { company?: string; email?: string; country?: string; whereSell?: string; volume?: string;
  isProduct?: boolean; productTitle?: string; productCategory?: string; productUrl?: string; message?: string; phone?: string; name?: string; appUrl?: string }) {
  const elements: any[] = [
    { tag: "div", fields: [
      { is_short: true, text: { tag: "plain_text", content: `${d.isProduct ? "客户/公司" : "公司"}: ${d.company || "-"}` } },
      { is_short: true, text: { tag: "plain_text", content: `邮箱: ${d.email || "-"}` } },
    ] },
  ];
  if (d.isProduct) {
    // 产品询盘：Joe 一眼看到问的哪个产品 + 留言
    elements.push({ tag: "div", text: { tag: "plain_text", content:
      `🛒 问的产品: ${d.productTitle || "-"}${d.productCategory ? " [" + d.productCategory + "]" : ""}\n` +
      `电话: ${d.phone || "-"} · 国家: ${d.country || "-"}\n` +
      `留言: ${d.message || "-"}` } });
  } else {
    elements.push({ tag: "div", text: { tag: "plain_text", content: `国家: ${d.country || "-"}\n在哪卖: ${d.whereSell || "-"}\n月走量: ${d.volume || "-"}` } });
  }
  if (d.appUrl) elements.push({ tag: "action", actions: [{ tag: "button", text: { tag: "plain_text", content: "打开后台跟进" }, url: d.appUrl, type: "primary" }] });
  return {
    msg_type: "interactive",
