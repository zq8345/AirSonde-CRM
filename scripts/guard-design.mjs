 ...
 *
 * ⛔⛔ **这道闸绿了 ≠ 这一页没问题。** 它只管一件事：**机器对 Joe 唠叨了多少**。
 *   它管不了的（2026-09-02 标出的盲区，记档不立单）：
 *     · **页面有多长** —— `[data-user-content]`（Joe 自己配的数据，如 26 个关键词 chip）
 *       按冻结口径永不计入预算，所以他每加一个词页面就更长一点，而这道闸永远是绿的。
 *     · 版式、对齐、可读性、要滚多久才看得到底 —— 那些的判据在 DESIGN.md 附录 C（见 C.9）。
 *   ⇒ 哪天 Joe 说"这页太长了"，**答案不在这道闸里**。别拿它的绿灯当"页面没问题"的证据。
 *   · 折叠块收起时的内容、(?) 提示内容不计
 *
