...
# 事后更正（2026-09-02，双源比对后追加；原文一字未动）

## 更正一 — 我漏了一条红级：rescore-taxonomy × scoreOnly 会批量重造 C5-33 僵尸

另一位审计员抓出、总工复验、**本窗亲验坐实**（`git show b166156:src/service.ts` 第 283 行区域）：

- 我审的版本上，C5-33 归位钩子裹在 `if (!opts.scoreOnly)` 里；
- 而 `/api/admin/rescore-taxonomy`（index.ts:1162）恰以 `scoreOnly: true` 打 **approved 集合**；
- ⇒ 排队中的全库重刷一跑，分数跌破 60 的 approved 线索**不会被归位**——批量重造"既不发也不退"的僵尸态。`scoreOnly` 的本意只是"不重写草稿"（service.ts:194），与状态归属无关——**旗子挂错了地方**。

**我为什么漏**：D 面结论「C5-33 钩子边界精确」评的是**谓词质量**（只降 approved、不碰 human_approved/queued/sent——这些都对），但**没有枚举"谁会以什么参数组合调用它"**。钩子的正确性 = 谓词 × 可达性，我只审了一半。
⇒ 记入宪章新判据：**审一个钩子/守卫，必须枚举其全部调用方的参数组合——被旗子跳过的守卫等于不存在。**
（生产尚未受损：重刷"排队未跑"，热修令已下，重刷前必堵。）

## 更正二 — 定级裁决记录：整点班整批发送升红

我在 🔵5 将"整点班/手动批量无间隔"记为"设计如此"。双源裁决**按红修**：宪法「就绪即执行，不等整点——**整点不是节奏，是实现细节**」（CRM-DESIGN.md §168-170）明文在先，整点班整批喷发与之相悖；我的 F 面抽了 5 条宪法-闸一致性，**没抽到这条最新的宪法条款**。事实层两审一致（我提供了生产实证：16 时 61 封 > 90s 间隔理论上限 40/时），分歧仅在定级——裁决采纳红级，发送收敛单一路径（并入 Y1 三合一热修）。

## 维持原判的部分

其余全部发现经双源比对一致或被采纳：372/276 根因（采纳本窗的一行修复）、sendable 三处同名不同义、failed=24 无出口、假定值 800、Y6 未推送（总工实测 25 个——比我数的 26 少 1，以他为准或含边界差异）。总评合议 **B**（本窗 B+ 与另一审 B- 的分歧即上述两条红的定级）。
