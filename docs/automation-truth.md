...
## 附注：已知的额外开销（报出来，别当它免费）

| 位置 | 开销 | 为什么接受 |
|---|---|---|
| `GET /api/leads` | 每次列表加载**多两条聚合查询**（当前筛选下的全量 COUNT + 早于重扫起点的条数），用于表格上方那行「这一格什么时候分析的」 | 1900 行量级上可忽略（D1 全表扫），总工 2026-09-05 明确接受。⚠️ 库涨到十万级时**要回来看这一条** —— 那时应改成"随 facets 一起算"或加索引，⛔ 不是删掉那行读数（Joe 两次追问要的就是它） |
| 整点班跟进 | 批内每封之间 `send_interval_seconds` 的等待占**轮次 wall time**（不占 CPU） | 循环里每次等之前先问预算（要留 `gapMs + 45s`），不够就停、剩下的下一班；⛔ 不硬撑过 Cron 的 15 分钟墙 |
